...
const CreateCampaignModal: React.FC<Props> = ({ onClose, onSuccess }) => {
    ...
    const [endDate, setEndDate] = useState(''); // Inicialmente vacío
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (type === 'discount' && selectedItems.length === 0) {
            alert("Debes seleccionar al menos un producto para aplicar el descuento.");
            return;
        }

        if (type === 'gift' && !rewardItem) {
            alert("Debes seleccionar un producto para regalar.");
            return;
        }
        // -----------------------------------------
        ...
        const payload = {
            ...
            endDate: new Date(endDate).toISOString(), // Ahora seguro porque validamos que no esté vacío
            config,
        ...
        try {
            ...
            console.error(error);
            alert(error.response?.data?.message || 'Error creando campaña');
        } finally {
            setLoading(false);
        }
    };
                        ...
                        <div className="border border-gray-200 rounded-xl max-h-48 overflow-y-auto p-2 space-y-2">
                            {myListings.length > 0 ? myListings.map(item => (
                                <div key={item.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg cursor-pointer" onClick={() => type === 'discount' ? toggleItem(item.id) : setRewardItem(item.id)}>
                                    <div className={`w-5 h-5 rounded flex items-center justify-center border ${(type === 'discount' ? selectedItems.includes(item.id) : rewardItem === item.id)
                                            ? 'bg-green-500 border-green-500 text-white'
                                            : 'border-gray-300'
                                        }`}>
                                        {((type === 'discount' && selectedItems.includes(item.id)) || (type === 'gift' && rewardItem === item.id)) && '✓'}
                                    </div>
                            ...
                            )) : <p className="text-center text-gray-400 py-4">No tienes productos activos para usar.</p>}
                        </div>
                    </div>
